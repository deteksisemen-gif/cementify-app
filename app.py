...
def init_session_state():
    ...
    if "history_table" not in st.session_state:
        st.session_state["history_table"] = pd.DataFrame(columns=["Waktu", "Merek", "Kuantitas", "Status", "Tautan Foto"])
    if "latest_scan" not in st.session_state:
        st.session_state["latest_scan"] = None
    if "latest_inventory" not in st.session_state:
        st.session_state["latest_inventory"] = {}

init_session_state()

# ==========================================
# 3. FUNGSI BACKEND: DATABASE & AI YOLOv8
# ==========================================
...
def save_to_google_sheets(data_list):
    try:
        if "gcp_service_account" in st.secrets:
            ...
            sheet = client.open("Database_Semen").sheet1 
...
def process_image(image_file, model):
    try:
        ...
        return processed_img, total_detected, inventory_counts
    except Exception as e:
        return None, 0, {}
    ...
    with col_kiri:
        ...
        st.markdown("""
            <div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 80px;'>
                ...
                <div style='display: flex; gap: 20px; font-size: 14px; color: #64748B; font-weight: 500;'>
                    <span>Features</span><span>Support</span><span>Contact</span>
                </div>
            </div>
            <h1 style='font-size: 42px; color: #0F172A; font-weight: 800; margin-bottom: 5px; text-align: center;'>Welcome Back!</h1>
            <p style='color: #64748B; margin-bottom: 40px; text-align: center;'>Please login to manage your construction projects</p>
        """, unsafe_allow_html=True)

        username = st.text_input("Username or Email Address", placeholder="admin@cementify.com")
        ...
        st.markdown("<div style='text-align: center; margin-top: 20px; color: #64748B; font-size: 14px;'>Don't have an account? <span style='color: #0F172A; font-weight: 700;'>Sign Up</span></div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_kanan:
        st.markdown("""
            <div style="
                ...
                border-radius: 30px 0 0 30px; box-shadow: -10px 0 20px rgba(0,0,0,0.05);
            "></div>
        """, unsafe_allow_html=True)

# ==========================================
# 5. HALAMAN DASHBOARD (DETEKSI YOLOv8)
# ==========================================
def show_dashboard_page():
    # Munculkan kembali sidebar dan kembalikan padding agar tidak mepet layar
    st.markdown("""
        <style>
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        menu = st.radio("Navigasi Utama", ["Capture & Upload", "History Log"])
        st.markdown("---")
    ...
    if menu == "Capture & Upload":
        ...
        with col_input:
            ...
            img_source = st.file_uploader("Upload Foto Gudang (JPG/PNG)", type=['jpg', 'jpeg', 'png', 'webp'])
            cam_source = st.camera_input("Atau Gunakan Kamera")
            active_source = img_source if img_source else cam_source
            ...
            if st.button("Proses Gambar Sekarang"):
                if not active_source:
                    ...
                    st.error("Model best.pt tidak ditemukan di folder!")
                else:
                    with st.spinner("Mesin YOLOv8 sedang menganalisis gambar..."):
                        result_img, total_sacks, inventory_counts = process_image(active_source, model)
                        
                        if result_img:
                            now = datetime.datetime.utcnow() + datetime.timedelta(hours=7)
                            time_str = now.strftime("%Y-%m-%d %H:%M")
                            
                            st.toast("Menyimpan ke Cloud...", icon="☁️")
                            img_url = upload_to_imgbb(result_img)
                            
                            st.session_state['latest_scan'] = result_img
                            st.session_state['latest_inventory'] = inventory_counts
                            
                            for brand, count in inventory_counts.items():
                                new_row = pd.DataFrame([{"Waktu": time_str, "Merek": brand, "Kuantitas": count, "Status": "✅ Selesai", "Tautan Foto": img_url}])
                            ...
                            details = ", ".join([f"{k}: {v}" for k, v in inventory_counts.items()])
                            save_to_google_sheets([time_str, total_sacks, details, img_url])
            st.markdown('</div>', unsafe_allow_html=True)

        with col_result:
            st.markdown("#### 2. Hasil Deteksi")
            if st.session_state['latest_inventory']:
                # Tampilkan Angka Total & Detail Merek
                total_current = sum(st.session_state['latest_inventory'].values())
                st.success(f"**Total Semen Terdeteksi: {total_current} Sak**")
                
                df_current = pd.DataFrame(list(st.session_state['latest_inventory'].items()), columns=["Merek Terdeteksi", "Jumlah Sak"])
                st.dataframe(df_current, hide_index=True, use_container_width=True)
                
                # Tampilkan Gambar Hasil Kotak-kotak YOLOv8
                st.image(st.session_state['latest_scan'], use_container_width=True, caption="Visualisasi Segmentasi Instance")
            else:
                st.info("Menunggu data pindaian dari operator...")

    elif menu == "History Log":
        st.title("Log Riwayat Pemindaian")
        st.markdown("---")
