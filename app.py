...
def show_login_page():
    ...
    with col_kiri:
        ...
        # Tombol Login Lebar Penuh dengan Validasi Username & Password
        if st.button("LOGIN", type="primary", use_container_width=True):
            # Username diubah ke huruf kecil (lower) agar tidak error walau diketik 'Admin'
            if username.lower() == "admin" and password == "indocement123":
                st.session_state['logged_in'] = True
                st.rerun()
            else:
                st.error("Kredensial tidak valid. Silakan periksa kembali Username dan Password Anda.")
